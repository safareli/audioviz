...
module.exports = function(){
    ...
    on.touch(['end','leave'], function (touches) {
        ...
        if(fullscreen.enabled() && this.list.length === 0){
            var startPerimeter = 0;
            var currentPerimeter = 0;
            var lastTime = this.queue[this.queue.length - 1].end;
            for (var i =0; i < this.queue.length; i++) {
                var touchCurrent  = this.queue[i],
                    touchNext = this.queue[i+1] || this.queue[0],
                    cdx = ~~(touchNext.pageX - touchCurrent.pageX),//currentDeltaX
                    cdy = ~~(touchNext.pageY - touchCurrent.pageY),//currentDeltaY
                    sdx = ~~(touchNext.start.pageX - touchCurrent.start.pageX),//startPerimeter
                    sdy = ~~(touchNext.start.pageY - touchCurrent.start.pageY);//startPerimeter
                if(lastTime - 100 < touchCurrent.end){
